                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h2 className="text-3xl font-black text-gray-900 dark:text-white tracking-tighter">
                            My <span className="text-[#FF6600]">Dashboard</span>
                        </h2>
                        <p className="text-gray-400 text-sm font-semibold">Manage your services and track your business.</p>
                    </div>
                    <Link
        ...
        >
            ...
            <div className="space-y-8">
                    ...
                    <div className="flex items-center justify-between">
                                    ...
                                    <>
                                        ...
                                        <div className="text-xs text-gray-500">
                                            {subscription.status === 'cancelled'
                                                ? `Cancelled — active until ${new Date(subscription.ends_at).toLocaleDateString()}`
                                                : `${subscription.remaining_days} days remaining · Expires ${new Date(subscription.ends_at).toLocaleDateString()}`
                                            }
                                        </div>
                                    </>
                                ) : (
                                    <>
                                        <div className="text-sm font-black text-gray-900 dark:text-white">No Active Subscription</div>
                                        <div className="text-xs text-gray-500">Subscribe to a plan to start listing services</div>
                                    </>
                                )}
                            </div>
                        </div>
                        {subscription ? (
                            subscription.status === 'active' && (
                                <button
                                    onClick={() => {
                                        if (confirm('Are you sure you want to cancel your subscription? You will still have access until the expiry date.')) {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    {[
                        { label: 'Total Services', value: totalServices, icon: Briefcase, color: '#FF6600' },
                        { label: 'Active Services', value: activeServices, icon: CheckCircle, color: '#22C55E' },
                        { label: 'Total Views', value: '—', icon: Eye, color: '#FFB800' },
                    ].map((stat, idx) => (
                        <motion.div
                            key={idx}
                            whileHover={{ y: -5 }}
                ...
                {services.length === 0 ? (
                    ...
                    >
                        ...
                        <h3 className="text-xl font-black text-gray-900 dark:text-white mb-2">Create Your First Service</h3>
                        <p className="text-gray-400 text-sm mb-6 max-w-md mx-auto">
                            Start getting customers by listing your services. Add details, set pricing, and go live!
                        </p>
                    ...
                    <div className="bg-white dark:bg-[#0c0c0c] p-8 rounded-[2.5rem] shadow-sm border border-gray-100 dark:border-white/5">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-xl font-bold text-gray-900 dark:text-white tracking-tight">Recent Services</h3>
